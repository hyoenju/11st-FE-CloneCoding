...
export default {
  data() {
    return {
      ...
      tabIndex: 0,
      myMenu: ['나의 쿠폰', '주문/배송조회', '취소/반품/교환', '고객센터', '회원정보'].map(
        item => { return {name: item, href: 'javascript:void(0)'}
      }), 
      headerMenu: ['베스트', '쿠폰/혜택', '기획전', '오늘장보기', 'T공식대리점', 'shocking-deal'].map(
        item => { return {name: item, href: 'javascript:void(0)'}
      }),
    }
  },
